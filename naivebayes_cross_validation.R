library('e1071')
classifier_naivebayes <- function (data_train, data_test, true_class, false_class){
  
  class <- factor(data_train[,'class'])
  trained_model_naive_bayes <- naiveBayes(data_train[,colnames(data_train) != 'class'], class)
  classification <- predict(trained_model_naive_bayes, data_test[,colnames(data_test) != 'class'])
...
crossValidation_k_folds_naivebayes <- function(data, number_folds, t, f){
  ...
  for (i in 1:number_folds){
    ...
    matrix_conf[[i]] <- classifier_naivebayes(data_train = folds_train, data_test = folds_test, t = t, f = f)
...
data <- data[,2:32]
k<-10
matrix_conf <- crossValidation_k_folds_naivebayes(data = data, number_folds = k, t = 'M', f = 'B')
